...
{
    if (!SetShaderParams(device_context, world_matrix, view_matrix, projection_matrix, texture))
        return false;
...
bool Shader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vertex_filename, WCHAR* pixel_filename)
{
    ...
    result = device->CreateInputLayout(input_layout, num_elements,
    ...
    result = device->CreateSamplerState(&sample_desc, &sampler_state_);
    if(FAILED(result))
        return false;
